  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setProximosPartidos(partidosData.filter(p => p.estado === 'pendiente').slice(0, 3));
        setResultados(partidosData.filter(p => p.estado === 'jugado').slice(0, 3));
  ...
  return (
    <div className="space-y-6">
      ...
      {isAdmin && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Link to="/pagos" className="bg-white p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow border-l-4 border-green-500">
            ...
            <p className=" text-sm">Gesttext-gray-500ionar contabilidad</p>
          </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <Link to="/partidos" className="bg-white rounded-xl shadow-md hover:shadow-lg transition-shadow overflow-hidden">
          ...
          <div className="p-4">
            {loading ? <p className="text-gray-500">Cargando...</p> : proximosPartidos.length === 0 ? (
              ...
              proximosPartidos.map((item, i) => (
                <div key={i} className="py-2 border-b last:border-b-0">
                  ...
                  <p className="text-sm text-gray-500">{item.fecha} {item.hora && `• ${item.hora}`}</p>
                </div>
              ))
            )}
          </div>
        ...
        <Link to="/resultados" className="bg-white rounded-xl shadow-md hover:shadow-lg transition-shadow overflow-hidden">
          ...
          <div className="p-4">
            {loading ? <p className="text-gray-500">Cargando...</p> : resultados.length === 0 ? (
              <p className="text-gray-500">No hay resultados</p>
            ) : (
              resultados.map((item, i) => (
                <div key={i} className="py-2 border-b last:border-b-0">
                  <p className="font-medium">{item.rival}</p>
                  <p className="text-sm text-gray-500">{item.fecha} • {item.resultado_local} - {item.resultado_visitante}</p>
                </div>
              ))
            )}
          </div>
        ...
        <Link to="/avisos" className="bg-white rounded-xl shadow-md hover:shadow-lg transition-shadow overflow-hidden">
          ...
          <div className="p-4">
            {loading ? <p className="text-gray-500">Cargando...</p> : avisos.length === 0 ? (
              <p className="text-gray-500">No hay avisos</p>
            ) : (
              avisos.map((item, i) => (
                <div key={i} className="py-2 border-b last:border-b-0">
                  <p className="font-medium">{item.titulo}</p>
                  <p className="text-sm text-gray-500">{item.fecha_publicacion?.split('T')[0]}</p>
                </div>
              ))
            )}
          </div>
